#!/usr/bin/env python3
"""
XGBoost Model for Multi-Target Prediction
...
print("="*60)
print("XGBOOST MODEL FOR AIR QUALITY PREDICTION")
print("="*60)
...
print("Saved: xgboost_results.json")
...
axes = axes.flatten()
fig.suptitle('XGBoost Model: Actual vs Predicted Values', fontsize=16, fontweight='bold')

for i, target in enumerate(target_columns):
    sample_size = min(500, len(y_test))
    axes[i].scatter(y_test.iloc[:sample_size, i], y_pred_test[:sample_size, i],
                    alpha=0.3, s=10, color='#ed8936')
...
axes = axes.flatten()
fig.suptitle('XGBoost Model: Time Series Predictions vs Actual', fontsize=16, fontweight='bold')
...
for i, target in enumerate(target_columns):
    ...
    axes[i].plot(y_pred_test[:sample_size, i], label='XGBoost Prediction',
...
axes[5].axis('off')
plt.tight_layout()
plt.savefig('xgboost_timeseries_predictions.png', dpi=150, bbox_inches='tight')
print("Saved: xgboost_timeseries_predictions.png")
plt.close()

# 3. Error distribution
fig, axes = plt.subplots(2, 3, figsize=(15, 10))
axes = axes.flatten()
fig.suptitle('XGBoost Model: Prediction Error Distributions', fontsize=16, fontweight='bold')

for i, target in enumerate(target_columns):
    errors = y_test.iloc[:, i].values - y_pred_test[:, i]
    axes[i].hist(errors, bins=50, color='#ed8936', alpha=0.7, edgecolor='black')
...
axes[5].axis('off')
plt.tight_layout()
plt.savefig('xgboost_error_distributions.png', dpi=150, bbox_inches='tight')
print("Saved: xgboost_error_distributions.png")
plt.close()
...
bars1 = ax.bar(x - width, mae_values, width, label='MAE', color='#ed8936')
...
ax.set_title('XGBoost Model: Performance by Target', fontsize=14, fontweight='bold')
ax.set_xticks(x)
...
plt.tight_layout()
plt.savefig('xgboost_performance_comparison.png', dpi=150, bbox_inches='tight')
print("Saved: xgboost_performance_comparison.png")
plt.close()
...
plt.barh(range(len(top_features)), top_features['importance'].values, color='#ed8936')
plt.yticks(range(len(top_features)), top_features['feature'].values)
plt.xlabel('Importance', fontsize=12)
plt.title('XGBoost Model: Top 15 Feature Importances for PM2.5 Prediction',
...
with open('xgboost_feature_importance.json', 'w') as f:
...
comparison_data['XGBoost'] = results

with open('model_comparison_all.json', 'w') as f:
    json.dump(comparison_data, f, indent=2)
print("Saved: model_comparison_all.json (updated)")
...
print("   - xgboost_features.json")
print("   - xgboost_metrics.json")
print("   - xgboost_results.json")
print("   - xgboost_feature_importance.json")
print("   - xgboost_*.png (5 visualization files)")
